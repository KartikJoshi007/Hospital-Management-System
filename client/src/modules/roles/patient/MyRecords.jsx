...
function MyRecords() {
      ...
      <tr key={record.id} className="hover:bg-slate-50/50 transition-all group">
         ...
         <td className="px-6 py-4">
            <div className="flex justify-center items-center gap-3">
               <button onClick={() => handleDownload(record)} disabled={downloadingId === record.id} className="p-2 text-slate-300 hover:text-emerald-500 hover:bg-emerald-50 rounded-xl transition-all">
                  {downloadingId === record.id ? <Loader2 size={16} className="animate-spin" /> : <Download size={16} strokeWidth={3} />}
               </button>
               <button onClick={() => handleViewDetail(record)} className="p-2 text-slate-300 hover:text-blue-500 hover:bg-blue-50 rounded-xl transition-all">
   ...
   return (
      <div className="space-y-6 pb-10 animate-in fade-in duration-700 w-full px-2 sm:px-4 max-w-[100vw] overflow-x-hidden">
         ...
         {/* 🔍 Filters & Quick Search - High Density */}
